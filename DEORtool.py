...
class DEOR_tool(object):
    ...
    def hp2az(n_side):
        # 方位角正东为0度，向北增加，直到2pi，高度角天顶为0，虽然不是很确定为什么会这样经过检查确实是这样
    ...
    def T2P(n_side, T, freq):
        k = 1.380649*10**-23  # boltzmann constant
        c = 3*10**8  # speed of light
        v = freq*1000000  # frequency
        I = (2*k*T*v**2)/(c**2)  # intensity
        s = (4*np.pi)/hp.nside2npix(n_side)  # solid angle ratio
        P = I * s
        return P
